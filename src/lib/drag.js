...
export default class Drag {
	...
	move(e) {
		...
		let offset = {
			x: translateX + movePosition.x - _self.startPosition.x,
			y: translateY + movePosition.y - _self.startPosition.y
		};
	...
	stopMove(e) {
		...
		let stopOffset = {
			x: translateX + stopPosition.x - _self.startPosition.x,
			y: translateY + stopPosition.y - _self.startPosition.y
		};
		...
		} else if (stopOffset.x >= 0 && stopOffset.y <= 0) {		// 右上角滑动
		...
		} else if (stopOffset.x <= 0 && stopOffset.y >= 0) {// 左下角滑动
		...
		} else if (stopOffset.x <= 0 && stopOffset.y <= 0) {// 左上角滑动
